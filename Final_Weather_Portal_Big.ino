...
#define NUM_BALLS         6                 //Number or raindrops/swnowflakes
int counterOne=0;
float temp=0;
...
int snow=0;
int iCon=6;
int numLeds=96;                             //Number of LEDs in your strip
#define PIN D5                               //Pin for the led output on WEMOS
int tell=1;
int sky[100];                                //array that holds clouds in position
int hold=0;
bool limiter=true;
//float timer=0;
int spot=0;
...
int pMax=0;
float timerFire=0;
Adafruit_NeoPixel strip = Adafruit_NeoPixel(96, PIN, NEO_GRBW + NEO_KHZ800);  //Usual setup for the Neopixels


/**
 * Wunderground Settings
 */
const String  WUNDERGRROUND_API_KEY = "Put your key here"; //you know the drill--you have to get a key
...
const char* WIFI_PASSWORD = "your password for your network"; //password
...
//This is all the cool information you can use if you want--I only used the temp and the current conditions....
  ...
  Serial.println(icon);
  timerTwo=millis();
  numLeds=map(temp,0,70,96,10);  //mapped the temp to height in pixels reversed--smaller flame for higher temp--its Alaska....
  numLeds=constrain(numLeds,10,96);
  iCon=icon;
  }
  //timer circuits for switching back between weather and temp every 30 seconds....
 for(int i=0;i<96;i++)strip.setPixelColor(i,0,0,0);  
...
void partlycloudy(){
   ...
   spot=random(5,60);
   spotPlus=random(6,20);
  for(int i=spot;i<spot+spotPlus;i++)sky[i]=1;
  
  //Serial.println("random");
  limiter=true;
  while(limiter){
for(int i=95;i>0;i=i-1){
  hold=sky[i-1];
  sky[i]=hold;
}
sky[0]=0;
for(int i=0;i<96;i++)strip.setPixelColor(i,0,0,255,i);
    for(int j=0;j<96;j++){
      //Serial.println(sky[j]);
      if(sky[j]==1){
        
        strip.setPixelColor(j,155,155,155,155);
        //strip.show();
      }
      else {
        
         // strip.setPixelColor(j,0,0,255,0);
          //strip.show();
...
sky[0]=0;
for(int i=0;i<96;i++)strip.setPixelColor(i,i*2,i*2,i*2);
    for(int j=0;j<96;j++){
      //Serial.println(sky[j]);
      if(sky[j]==1){
...
void rainstorm(){
  ...
  for(int i=0;i<96;i++)strip.setPixelColor(i,0,25,12,0);
  for(int i=0;i<pMax;i++)strip.setPixelColor(i,0,255,i*4);
  for(int i=0;i<NUM_BALLS;i++){
    strip.setPixelColor(pos[i],0,255,0,0);
...
void snowstorm(){
  ...
  for(int i=0;i<96;i++)strip.setPixelColor(i,0,0,50);
  for(int i=0;i<pMax;i++)strip.setPixelColor(i,i*5,i*5,i*5,i*5);
  for(int i=0;i<NUM_BALLS;i++){
